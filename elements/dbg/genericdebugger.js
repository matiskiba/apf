if (apf.hasRequireJS) require.def("apf/elements/dbg/genericdebugger",
    ["debug/Breakpoint"],
    function(Breakpoint) {

var GenericDebugger = function(dbg, host) {
    this.$init();
...
(function() {
    ...
    this.setBreakpoints = function(model, callback) {
		...
		console.debug("BREAK POINTS " + breakpoints);
        _self.$debugger.listbreakpoints(function(v8Breakpoints) {
			// TODO: remove line. added it as an hack so I can pass null to the callback
			if (v8Breakpoints)
			...
			console.debug("BREAK POINTS " + modelBps);
...
}).call(GenericDebugger.prototype = new apf.Class());

return GenericDebugger;
